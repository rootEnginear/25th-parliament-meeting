...
for (entry in raw_data) {
  ...
  */
  ...
  const title = [...document.querySelectorAll("#mydetail > td")[0].children]
    .map((el) => el.textContent.trim())
    .filter((el) => el.length > 0);
  const score_summary_docs = [
    ...document.querySelectorAll("#mydetail_o > td > ul > li > a[target]"),
  ].map((el) => [
    el.textContent.trim(),
    (
      "https://msbis.parliament.go.th/ewtadmin/ewt/parliament_report/" +
      el.href.replace("../parliament_report/", "")
    ).trim(),
  ]);
  const meeting_docs = [
    ...document.querySelectorAll("table > tbody > tr > td > a[target]"),
  ].map((el) => [
    el.textContent.trim(),
